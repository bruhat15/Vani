...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const setAuthSession = useVaniAppStore((state) => state.setAuthSession);
  const clearAuthSession = useVaniAppStore((state) => state.clearAuthSession);

  const updateAuthState = useCallback((newSession: Session | null) => {
    console.log('AuthContext: Updating auth state:', newSession?.user?.email || 'No session');
    setSession(newSession);
    setUser(newSession?.user ?? null);
    sessionAccessTokenRef.current = newSession?.access_token ?? null;
  ...
  const clearAuthState = useCallback(() => {
    ...
    setUser(null);
    setError(null);
    sessionAccessTokenRef.current = null;
    clearAuthSession();
  }, [clearAuthSession]);
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, newSession) => {
        ...
        if (sessionAccessTokenRef.current !== (newSession?.access_token ?? null)) {
          updateAuthState(newSession);
          setLoading(false);
        }
      }
